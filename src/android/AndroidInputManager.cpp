...
KeyEvent AndroidKeyboard::convertKey(int action, int keyCode)
{
        ...
        switch(keyCode){
        case 4:  //Added for android back event!!
                kc = OIS::KC_WEBBACK;
                break;
        case 7:
                kc = OIS::KC_0;
                ...
                mWidth(100), mHeight(100)
{
        for (int i = 0; i < OIS_MAX_NUM_TOUCHES; i++) { 
          mStates.push_back( OIS::MultiTouchState() );
        }
}

//copied from ogre3d android samplebrower
void AndroidMultiTouch::injectTouch(int action, float x, float y, int touchId, float pressure)
{
        assert(!mStates.empty());
        if (touchId >= OIS_MAX_NUM_TOUCHES) return;

        OIS::MultiTouchState &state = mStates[touchId];
